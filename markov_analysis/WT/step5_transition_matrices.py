...
=========================================================
MSM pipeline for WT Nav1.5 (Martini 3 CG).

**Block 2 is excluded** — only blocks 1 and 3 are used, as block 2
(1,960 frames) has poor internal connectivity (largest SCC = 9).
...
def load_and_aggregate(count_dir, block_names, lag, n_states):
    """Load per-block count matrices and sum (block 2 excluded)."""
...
def plot_eigenvalue_spectrum(ev, lag, outdir):
    ...
    ax.set_title(f"MSM Eigenvalue Spectrum (τ={lag}, reversible, "
                 f"blocks 1+3) — WT Nav1.5", fontsize=13)
    ax.set_ylim(0, 1.05)
...
def plot_implied_timescales_table(ev, lag, dt_ps, outdir):
    ...
    ax.set_title(f"Implied Timescales (τ={lag}, blocks 1+3) — WT Nav1.5",
                 fontsize=13)
    ax.grid(True, alpha=0.3, axis="y")
    plt.tight_layout()
...
def plot_stationary_distribution(pi, scc_map, outdir):
    ...
    fig.suptitle("Stationary Distribution — WT Nav1.5 (blocks 1+3)",
                 fontsize=14)
    plt.tight_layout()
...
def plot_transition_matrix_heatmap(T, outdir, tag="reversible"):
    ...
    ax.set_title(f"Transition Matrix T(τ) [{tag}] — WT Nav1.5", fontsize=13)
    plt.tight_layout()
...
def plot_ck_test(ck_results, dt_ps, outdir):
    ...
    ax.set_title("Chapman-Kolmogorov Test — WT Nav1.5 (blocks 1+3)",
                 fontsize=13)
...
def main():
    ...
    print("=" * 70)
    print("  Step 5 — Transition Probability Matrices  |  WT Nav1.5  (CG)")
    print("=" * 70)
    print(f"  Blocks used      : {', '.join(BLOCKS)}  (block2 excluded)")
    ...
    # ── 1. Load & aggregate counts (blocks 1 + 3) ──
    print(f"\n[1] Loading count matrices (block2 excluded) …")
    C_agg = load_and_aggregate(COUNT_DIR, BLOCKS, lag, n_states)
    np.save(os.path.join(OUT, f"C_agg_b13_tau{lag}.npy"), C_agg)
    ...
    with open(summary_path, "w") as f:
        ...
        f.write("=" * 60 + "\n\n")
        f.write(f"Blocks used        : {', '.join(BLOCKS)} (block2 excluded)\n")
        ...
        f.write(f"\nCount matrix (blocks 1+3 aggregated, SCC):\n")
        ...
        f.write(f"\nOutputs in {OUT}/:\n")
        f.write(f"  C_agg_b13_tau{lag}.npy          — aggregated counts (full)\n")
