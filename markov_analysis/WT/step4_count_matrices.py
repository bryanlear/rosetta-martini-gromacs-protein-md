...
def plot_implied_timescales(scan, n_show, outdir, tag="aggregated"):
    ...
    ax.set_title(f"Implied Timescales ({tag}) — WT Nav1.5", fontsize=14)
...
def plot_implied_timescales_per_block(scan, n_show, outdir):
    ...
    fig.suptitle("Per-Block Implied Timescales — WT Nav1.5", fontsize=14)
    plt.tight_layout()
...
def plot_count_matrix_heatmap(C, outdir, tag="aggregated"):
    ...
    ax.set_title(f"Count Matrix C(τ) [{tag}] — WT Nav1.5", fontsize=13)
    plt.tight_layout()
...
def plot_eigenvalue_spectrum(eigenvalues, lag, outdir, tag="aggregated"):
    ...
    ax.set_title(f"MSM Eigenvalue Spectrum (τ={lag}, {tag}) — WT Nav1.5",
                 fontsize=13)
    ax.set_ylim(0, 1.05)
...
def plot_stationary_distribution(T, outdir, tag="aggregated"):
    ...
    ax.set_title(f"Stationary Distribution ({tag}) — WT Nav1.5", fontsize=13)
    ax.grid(True, alpha=0.3, axis="y")
    plt.tight_layout()
...
def main():
    ...
    print("=" * 70)
    print("  Step 4 — Local Transition Count Matrices  |  WT Nav1.5  (CG)")
    print("=" * 70)
