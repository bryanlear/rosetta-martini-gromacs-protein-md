...
def plot_elbow(scan, outdir):
    ...
    ax.set_title("k-Medoids Elbow Plot — WT Nav1.5", fontsize=14)
...
def plot_silhouette_vs_k(scan, outdir):
    ...
    ax.set_title("Silhouette Score vs k — WT Nav1.5", fontsize=14)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_cluster_map(X_tica, labels, medoid_idx, outdir, filename="kmedoids_tic1_vs_tic2.png"):
    ...
    ax.set_title(f"k-Medoids Clustering (k={n_clusters}) — WT Nav1.5", fontsize=14)
...
def plot_block_timeseries(block_labels, block_names, dt_ps, outdir):
    ...
    fig.suptitle("Microstate Time Series — WT Nav1.5", fontsize=14, y=1.01)
    plt.tight_layout()
...
def main():
    ...
    print("=" * 65)
    print("  Step 3 — k-Medoids Clustering  |  WT Nav1.5  (Martini 3 CG)")
    print("=" * 65)
    print(f"  Final k          : {k}")
